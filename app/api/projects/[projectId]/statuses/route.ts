...
export async function GET(_: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  const statuses = await prisma.status.findMany({ where: { projectId: projectId, deletedAt: null }, orderBy: { position: "asc" } });
...
export async function POST(req: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  const parsed = issueSchemas.createStatus.safeParse({ ...body, projectId: projectId });
  if (!parsed.success) return NextResponse.json({ error: "Invalid input" }, { status: 400 });
  try {
    const status = await createStatus(projectId, session.user.id, parsed.data.name, parsed.data.color, parsed.data.wipLimit);
...
export async function PATCH(req: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  const session = await requireSession();
  const body = await req.json().catch(() => null);
  const parsed = issueSchemas.reorderStatuses.safeParse({ ...body, projectId: projectId });
  if (!parsed.success) return NextResponse.json({ error: "Invalid input" }, { status: 400 });
  try {
    await reorderStatuses(projectId, session.user.id, parsed.data.order);
