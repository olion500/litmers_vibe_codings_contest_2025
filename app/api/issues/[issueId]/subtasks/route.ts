...
export async function POST(req: Request, { params }: { params: { issueId: string } }) {
  ...
  const parsed = issueSchemas.subtask.safeParse({ ...body, issueId: params.issueId });
  if (!parsed.success) return NextResponse.json({ error: "Invalid input" }, { status: 400 });
  try {
    const subtask = await createSubtask(params.issueId, session.user.id, parsed.data.title);
...
export async function PATCH(req: Request, { params }: { params: { issueId: string } }) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Invalid input" }, { status: 400 });
  const subtask = await prisma.subtask.findFirst({ where: { id: parsed.data.subtaskId, issueId: params.issueId } });
