...
export async function GET(_: Request, { params }: { params: { issueId: string } }) {
  const session = await requireSession();
  const issue = await prisma.issue.findFirst({
    where: { id: params.issueId, deletedAt: null, project: { team: { members: { some: { userId: session.user.id, deletedAt: null } } } } },
    include: {
      status: true,
...
export async function PATCH(req: Request, { params }: { params: { issueId: string } }) {
  ...
  try {
    const updated = await updateIssue(params.issueId, session.user.id, parsed.data);
...
export async function DELETE(_: Request, { params }: { params: { issueId: string } }) {
  const session = await requireSession();
  try {
    await softDeleteIssue(params.issueId, session.user.id);
